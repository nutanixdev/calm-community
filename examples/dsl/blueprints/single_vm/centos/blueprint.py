...
from calm.dsl.builtins import AhvVmResources, ahv_vm, vm_disk_package
...
# Credentials
OS_USER = "nutanix"
OS_KEY = read_local_file(".tests/keys/centos")
...
class Profile1(VmProfile):
    ...
    provider_spec = ahv_vm(resources=SingleVmAhvResources, name="@@{calm_application_name}@@")
    ...
    def __install__():
        CalmTask.Exec.ssh(
            name="DOCKER_INSTALL",
            script=requests.get(SCRIPTS_URL + "/linux/docker/centos_docker_install.sh").text
        )
